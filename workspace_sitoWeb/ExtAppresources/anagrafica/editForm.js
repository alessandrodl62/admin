...
Anagrafica.form.EditForm = function(config){



	var storeComboSex =  new Ext.data.SimpleStore({
		fields: ['sesso','descrizioneSex'],
		data: [['M','Maschio'],['F','Femmina'],['G','Giuridico']]
	...
	var idAnagProf = {xtype:'hidden', name:'idAnagProf'}
	var idAdresse = {xtype:'hidden', name:'idAdresse'}
	...
	this.saveAction = function(){
		...
		_this.getForm().submit({
			url:'/commongest/CommonServlet/anagrafica/salvaanag',
			success:function(){
...
/*		title:'dati tecnici',*/		
	...
	Anagrafica.form.EditForm.superclass.constructor.call(this, Ext.apply({},  config , defaults))
}

	
Ext.extend(Anagrafica.form.EditForm, Ext.form.FormPanel);

//registro l'oggetto EditForm
Ext.reg('anagraficaformedit', Anagrafica.form.EditForm)
